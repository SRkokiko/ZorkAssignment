...
- Return `const std::list<Entity*>&` for container getters; a `const` list reference still gives non-const pointers so callers can mutate the pointed-to objects
- Only add a non-const overload if callers genuinely need to modify the container structure (add/remove items) and there is no better explicit method (e.g. `AddEntity`, `RemoveEntity`)
